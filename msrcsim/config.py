...
def _set_output_defaults(d):
    ...
    d['output'].setdefault('record_gene_trees', True)
...
def load_config(path):
    ...
    if mode == 'mechanistic':
        ...
        if missing:
            raise ValueError(f"Missing mechanistic configuration sections: {missing}")
    else:
        if 'structured_interval' not in d:
            raise ValueError("Conditional mode requires 'structured_interval'")
        si = d['structured_interval']
        for key in ['duration', 'configuration', 'migration', 'coalescence']:
            if key not in si:
                raise ValueError(f"Conditional structured_interval missing {key!r}")
        if len(str(si['configuration'])) != 4 or any(c not in '01' for c in str(si['configuration'])):
            raise ValueError("Conditional configuration must be a four-character binary string")
