...
def main() -> None:
    ...
    for attempt,ss in enumerate(master.spawn(max_attempts)):
        ...
        hist=simulate_population_history(tree,rearr,rng,config)
        ...
        if result.accepted:
            save_frozen_history(args.output,config,hist,sampled,{"attempt_id":attempt,"history_seed":int(ss.generate_state(1)[0]),"terminal_pattern":terminal_pattern(sampled,tree.taxa),"conditioning_reason":result.reason,"population_process":resolved_population_process(config)})
